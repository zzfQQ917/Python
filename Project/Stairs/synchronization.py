...
class stairs:
    ...
    def craft_game(self):
        ...
        while True:
            choice = int(input("초대할 사람을 선택하세요: "))
            if 0 <= choice <= len(player_list) - 1:
                break
        
        opponent = player_list[choice]['id']
        room_info['player2'] = opponent
        
        games.insert_one(room_info)       
       
       
    def test_game(self):
        rooms = list(games.find())
        room_dict = {}
        
        for i, room in enumerate(rooms):
            print(f"방 번호: {room['code']} | 참가자: {room['player1']}. {room['player2']}")
            room_dict[room['code']] = room
        
        while True:        
            choice = input("방 번호를 선택하세요: ")
            if choice not in room_dict.keys():
                print("번호에 해당되는 방이 없습니다")
                continue
            elif not ((room_dict[choice]['player1'] == self.id) or (room_dict[choice]['player2'] == self.id)):
                print("방에 참가하지 않았습니다")
                continue
            break
            
        target_room = room_dict[choice]
        target_room_id = target_room['code']
        
        if target_room['player1'] == self.id:
            first = True
        else:
            first = False
        ...
        while True:
            if first == False:
                while True:
                    if self.load_room(target_room_id)['player1_choice']:
                        break
                    else:
                        print("Player1이 선택할 때까지 기다리는 중입니다.")
                        time.sleep(2)
                        continue
                        
                rsp = self.check_rsp_choice()
                games.update_one({
                    "code": target_room_id
                }, {
                    "$set": {
                ...
                while True:
                    if self.load_room(target_room_id)['game_continue']:
                        break
                    else:
                        print("Player2가 선택할 때까지 기다리는 중입니다.")
                        time.sleep(2)
                        continue
    ...
    def load_room(self, id):
        return games.find_one({"code": id})
    
    def check_rsp_choice(self):
        while True:
            choice = input("가위, 바위, 보 중 선택: ")
